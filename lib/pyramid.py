...
def process_multiscale(image, model, scales=[.25, 0.50, 1.0]):
    ...
    all_descriptors = torch.zeros([
        model.dense_feature_extraction.num_channels, 0  # num_channels 应该算D2Net 的最后一层的通道数
    ])
    ...
    for idx, scale in enumerate(scales):
        # 双线性插值
        # 如果想看生成的current_image 可以这样处理然后看np.array(current_image.squeeze().cpu()).transpose(1,2,0).shape
        # 用matplotlib.pyplot 显示图像
